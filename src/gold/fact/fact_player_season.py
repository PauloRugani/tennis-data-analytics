...
def run_transformation(spark, tb_player_match, tb_tournaments, tb_players):
    try:
...
def run():
    ...
    try:
        ...
        tb_player_match, tb_tournaments, tb_players = load_tables(spark)
        df_final = run_transformation(spark, tb_player_match, tb_tournaments, tb_players)
        save_table(df_final)
    finally:
        if spark:
