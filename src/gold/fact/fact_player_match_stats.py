...
def load_tables(handler, bucket_name):
    try:
        logger.info("Loading fact_player_match_stats source tables...")
        ...
        logger.error(f"Failed to load fact_player_match_stats source tables: {e}")
        raise

def run_transformation(tb_player_match, tb_tournaments, tb_date, tb_players):
    try:
        df = (
        ...
        logger.error(f"Failed to transform fact_player_match_stats: {e}")
        raise
...
def save_table(handler, df, bucket_name, jdbc_url, jdbc_user, jdbc_password):
    try:
        ...
        logger.error(f"Failed to save fact_player_match_stats: {e}")
        raise

def run(conn_vars: dict = None):
    handler = None
    try:
        logger.info("Starting fact_player_match_stats run")
        ...
        df_final = run_transformation(tb_player_match, tb_tournaments, tb_date, tb_players)
        save_table(handler, df_final, bucket_name, jdbc_url, jdbc_user, jdbc_password)
        logger.info("Finished fact_player_match_stats run")
    finally:
