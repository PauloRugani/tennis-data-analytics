...
def load_tables(handler):
    try:
        ...
        tb_date = handler.load_data(
            spark=handler.spark,
            path=f"s3a://{os.getenv('MINIO_BUCKET')}/gold/dimension/dim_date/",
            format="parquet"
        )
        tb_entry = handler.load_data(
            spark=handler.spark,
            path=f"s3a://{os.getenv('MINIO_BUCKET')}/gold/dimension/dim_entry/",
            format="parquet"
        )
...
def save_table(handler, df):
    try:
        handler.save_data(
            df=df,
            path=f"s3a://{os.getenv('MINIO_BUCKET')}/gold/fact/fact_player_tournament_stats/",
            format="parquet",
            mode="overwrite"
        )
